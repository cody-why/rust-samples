...
use ice_rs_demo::gen::demo::{ContactDB,ContactDBPrx,NumberType};
...
async fn run(comm: &mut Communicator) -> Result<(), Box<dyn std::error::Error + Send + Sync>> {
    ...
    if dial_group.is_none() || dial_group.unwrap() != 0 {
    ...
    if info.num_type != Some(NumberType::HOME) {
    ...
    if info.number != frank_number || info.num_type != Some(NumberType::CELL) {
        print!("info is incorrect ");
        return Ok(())
    }
    ...
    if info.num_type != Some(NumberType::OFFICE) || info.dial_group != Some(2) {
    ...
    let info = contact_db.query(&String::from("anne"), None).await?;
    if info.number != anne_number || info.num_type != Some(NumberType::OFFICE) || info.dial_group != Some(2) {
        print!("info is incorrect ");
        return Ok(())
    }
