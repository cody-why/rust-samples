...
use async_std::{io::{self, ReadExt, WriteExt}, net::{TcpListener, TcpStream}, stream::StreamExt, task};
...
async fn handle(mut src_stream: TcpStream, addr: SocketAddr) -> Result<(), Box<dyn std::error::Error>> {
    ...
    // let mut src_reader =  io::BufReader::new(src_reader);
    // let mut dst_reader =  io::BufReader::new(dst_reader);
    // let mut src_writer = io::BufWriter::new(src_writer);
    // let mut dst_writer = io::BufWriter::new(dst_writer);
