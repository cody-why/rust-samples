...
async fn handle(mut src_stream: tokio::net::TcpStream, addr: SocketAddr) -> Result<(), Box<dyn std::error::Error>> {
    ...
    // let mut src_reader =  tokio::io::BufReader::new(src_reader);
    ...
    let c_to_s = tokio::spawn(async move {
        ...
        loop {
            let n = src_reader.read(&mut buf).await.map_err(|e| {
                info!("c_to_s {} read error: {}", addr, e);
                e
            })?;
            if n == 0 {
                break;
            }
            dst_writer.write_all(&buf[0..n]).await?;
            dst_writer.flush().await?;
    ...
    let s_to_c = tokio::spawn(async move {
        ...
        // let mut buf = vec![0x00; 4096];
        // loop {
        //     let n = dst_reader.read(&mut buf).await.map_err( |e| {
        //         info!("s_to_c {} read error: {}", addr, e);
        //         e
        //     })?;
        //     if n == 0 {
        //         break;
        //     }
        //     src_writer.write_all(&buf[0..n]).await?;
        //     src_writer.flush().await?;
