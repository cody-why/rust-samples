...
impl ToBytes for Contact {
    fn to_bytes(&self) -> Result<Vec<u8>, Box<dyn std::error::Error + Send + Sync>> {
        ...
        bytes.extend(OptionalWrapper::new(1u8, self.r#type.clone()).to_bytes()?);
    ...
    async fn handle_request(
        ...
        match request.operation.as_ref() {
            ...
            "addContact" => {
                ...
                let result = self
                    .server_impl
                    .add_contact(&name, r#type, number, dial_group, None)
                    .await;
            ...
            "updateContact" => {
                ...
                let result = self
                    .server_impl
                    .update_contact(&name, r#type, number, dial_group, None)
                    .await;
                let wrapped_result = result;
                let result = wrapped_result.to_bytes()?;
                Ok(ReplyData {
